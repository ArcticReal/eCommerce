...
public class FindInventoryItemAttributesBy extends Query {


Map<String, String> filter;
public FindInventoryItemAttributesBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<InventoryItemAttribute> foundInventoryItemAttributes = new ArrayList<InventoryItemAttribute>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("inventoryItemAttributeId")) { 
 GenericValue foundElement = delegator.findOne("InventoryItemAttribute", false, filter);
...
throw new RecordNotFoundException(InventoryItemAttribute.class); 
 ...
 buf = delegator.findAll("InventoryItemAttribute", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundInventoryItemAttributes.add(InventoryItemAttributeMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new InventoryItemAttributeFound(foundInventoryItemAttributes);
