...
public class FindShipmentStatussBy extends Query {


Map<String, String> filter;
public FindShipmentStatussBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<ShipmentStatus> foundShipmentStatuss = new ArrayList<ShipmentStatus>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("shipmentStatusId")) { 
 GenericValue foundElement = delegator.findOne("ShipmentStatus", false, filter);
...
throw new RecordNotFoundException(ShipmentStatus.class); 
 ...
 buf = delegator.findAll("ShipmentStatus", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundShipmentStatuss.add(ShipmentStatusMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new ShipmentStatusFound(foundShipmentStatuss);
