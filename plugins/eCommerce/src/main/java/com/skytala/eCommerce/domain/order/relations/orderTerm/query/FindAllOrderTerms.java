...
public class FindAllOrderTerms extends Query {

@Override
public Event execute() {

Delegator delegator = DelegatorFactory.getDelegator("default");
List<OrderTerm> returnVal = new ArrayList<OrderTerm>();
try{
List<GenericValue> results = delegator.findAll("OrderTerm", false);
for (int i = 0; i < results.size(); i++) {
returnVal.add(OrderTermMapper.map(results.get(i)));
}
} catch (GenericEntityException e) {
 System.err.println(e.getMessage()); 
}
Event resultingEvent = new OrderTermFound(returnVal);
