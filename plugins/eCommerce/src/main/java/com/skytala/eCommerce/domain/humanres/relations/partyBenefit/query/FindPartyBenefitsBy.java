...
public class FindPartyBenefitsBy extends Query {


Map<String, String> filter;
public FindPartyBenefitsBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<PartyBenefit> foundPartyBenefits = new ArrayList<PartyBenefit>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("partyBenefitId")) { 
 GenericValue foundElement = delegator.findOne("PartyBenefit", false, filter);
...
throw new RecordNotFoundException(PartyBenefit.class); 
 ...
 buf = delegator.findAll("PartyBenefit", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundPartyBenefits.add(PartyBenefitMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new PartyBenefitFound(foundPartyBenefits);
