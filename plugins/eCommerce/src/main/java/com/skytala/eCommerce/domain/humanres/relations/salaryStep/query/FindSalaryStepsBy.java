...
public class FindSalaryStepsBy extends Query {


Map<String, String> filter;
public FindSalaryStepsBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<SalaryStep> foundSalarySteps = new ArrayList<SalaryStep>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("salaryStepId")) { 
 GenericValue foundElement = delegator.findOne("SalaryStep", false, filter);
...
throw new RecordNotFoundException(SalaryStep.class); 
 ...
 buf = delegator.findAll("SalaryStep", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundSalarySteps.add(SalaryStepMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new SalaryStepFound(foundSalarySteps);
