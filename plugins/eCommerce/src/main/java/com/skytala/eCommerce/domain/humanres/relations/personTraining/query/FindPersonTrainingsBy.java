...
public class FindPersonTrainingsBy extends Query {


Map<String, String> filter;
public FindPersonTrainingsBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<PersonTraining> foundPersonTrainings = new ArrayList<PersonTraining>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("personTrainingId")) { 
 GenericValue foundElement = delegator.findOne("PersonTraining", false, filter);
...
throw new RecordNotFoundException(PersonTraining.class); 
 ...
 buf = delegator.findAll("PersonTraining", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundPersonTrainings.add(PersonTrainingMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new PersonTrainingFound(foundPersonTrainings);
