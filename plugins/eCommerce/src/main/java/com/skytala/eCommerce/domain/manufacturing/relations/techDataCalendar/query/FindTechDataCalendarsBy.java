...
public class FindTechDataCalendarsBy extends Query {


Map<String, String> filter;
public FindTechDataCalendarsBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<TechDataCalendar> foundTechDataCalendars = new ArrayList<TechDataCalendar>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("techDataCalendarId")) { 
 GenericValue foundElement = delegator.findOne("TechDataCalendar", false, filter);
...
throw new RecordNotFoundException(TechDataCalendar.class); 
 ...
 buf = delegator.findAll("TechDataCalendar", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundTechDataCalendars.add(TechDataCalendarMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new TechDataCalendarFound(foundTechDataCalendars);
