...
public class FindAllOrderBlacklists extends Query {

@Override
public Event execute() {

Delegator delegator = DelegatorFactory.getDelegator("default");
List<OrderBlacklist> returnVal = new ArrayList<OrderBlacklist>();
try{
List<GenericValue> results = delegator.findAll("OrderBlacklist", false);
for (int i = 0; i < results.size(); i++) {
returnVal.add(OrderBlacklistMapper.map(results.get(i)));
}
} catch (GenericEntityException e) {
 System.err.println(e.getMessage()); 
}
Event resultingEvent = new OrderBlacklistFound(returnVal);
