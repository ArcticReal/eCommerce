...
public class FindWorkOrderItemFulfillmentsBy extends Query {


Map<String, String> filter;
public FindWorkOrderItemFulfillmentsBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<WorkOrderItemFulfillment> foundWorkOrderItemFulfillments = new ArrayList<WorkOrderItemFulfillment>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("workOrderItemFulfillmentId")) { 
 GenericValue foundElement = delegator.findOne("WorkOrderItemFulfillment", false, filter);
...
throw new RecordNotFoundException(WorkOrderItemFulfillment.class); 
 ...
 buf = delegator.findAll("WorkOrderItemFulfillment", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundWorkOrderItemFulfillments.add(WorkOrderItemFulfillmentMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new WorkOrderItemFulfillmentFound(foundWorkOrderItemFulfillments);
