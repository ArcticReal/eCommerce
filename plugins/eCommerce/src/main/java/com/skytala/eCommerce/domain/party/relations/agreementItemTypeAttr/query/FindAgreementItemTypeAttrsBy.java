...
public class FindAgreementItemTypeAttrsBy extends Query {


Map<String, String> filter;
public FindAgreementItemTypeAttrsBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<AgreementItemTypeAttr> foundAgreementItemTypeAttrs = new ArrayList<AgreementItemTypeAttr>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("agreementItemTypeAttrId")) { 
 GenericValue foundElement = delegator.findOne("AgreementItemTypeAttr", false, filter);
...
throw new RecordNotFoundException(AgreementItemTypeAttr.class); 
 ...
 buf = delegator.findAll("AgreementItemTypeAttr", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundAgreementItemTypeAttrs.add(AgreementItemTypeAttrMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new AgreementItemTypeAttrFound(foundAgreementItemTypeAttrs);
