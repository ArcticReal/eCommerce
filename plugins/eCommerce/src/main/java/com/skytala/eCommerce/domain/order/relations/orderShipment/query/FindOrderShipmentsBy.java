...
public class FindOrderShipmentsBy extends Query {


Map<String, String> filter;
public FindOrderShipmentsBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<OrderShipment> foundOrderShipments = new ArrayList<OrderShipment>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("orderShipmentId")) { 
 GenericValue foundElement = delegator.findOne("OrderShipment", false, filter);
...
throw new RecordNotFoundException(OrderShipment.class); 
 ...
 buf = delegator.findAll("OrderShipment", false); 
 }
...
Event resultingEvent = new OrderShipmentFound(foundOrderShipments);
