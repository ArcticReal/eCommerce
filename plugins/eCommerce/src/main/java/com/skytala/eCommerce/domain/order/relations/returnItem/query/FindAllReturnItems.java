...
public class FindAllReturnItems extends Query {

@Override
public Event execute() {

Delegator delegator = DelegatorFactory.getDelegator("default");
List<ReturnItem> returnVal = new ArrayList<ReturnItem>();
try{
List<GenericValue> results = delegator.findAll("ReturnItem", false);
for (int i = 0; i < results.size(); i++) {
returnVal.add(ReturnItemMapper.map(results.get(i)));
}
} catch (GenericEntityException e) {
 System.err.println(e.getMessage()); 
}
Event resultingEvent = new ReturnItemFound(returnVal);
