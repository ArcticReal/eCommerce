...
public class FindSurveyQuestionApplsBy extends Query {


Map<String, String> filter;
public FindSurveyQuestionApplsBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<SurveyQuestionAppl> foundSurveyQuestionAppls = new ArrayList<SurveyQuestionAppl>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("surveyQuestionApplId")) { 
 GenericValue foundElement = delegator.findOne("SurveyQuestionAppl", false, filter);
...
throw new RecordNotFoundException(SurveyQuestionAppl.class); 
 ...
 buf = delegator.findAll("SurveyQuestionAppl", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundSurveyQuestionAppls.add(SurveyQuestionApplMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new SurveyQuestionApplFound(foundSurveyQuestionAppls);
