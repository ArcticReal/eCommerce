...
public class FindAllPartySkills extends Query {

@Override
public Event execute() {

Delegator delegator = DelegatorFactory.getDelegator("default");
List<PartySkill> returnVal = new ArrayList<PartySkill>();
try{
List<GenericValue> results = delegator.findAll("PartySkill", false);
for (int i = 0; i < results.size(); i++) {
returnVal.add(PartySkillMapper.map(results.get(i)));
}
} catch (GenericEntityException e) {
 System.err.println(e.getMessage()); 
}
Event resultingEvent = new PartySkillFound(returnVal);
