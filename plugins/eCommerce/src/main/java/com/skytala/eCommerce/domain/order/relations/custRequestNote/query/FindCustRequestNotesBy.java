...
public class FindCustRequestNotesBy extends Query {


Map<String, String> filter;
public FindCustRequestNotesBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<CustRequestNote> foundCustRequestNotes = new ArrayList<CustRequestNote>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("custRequestNoteId")) { 
 GenericValue foundElement = delegator.findOne("CustRequestNote", false, filter);
...
throw new RecordNotFoundException(CustRequestNote.class); 
 ...
 buf = delegator.findAll("CustRequestNote", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundCustRequestNotes.add(CustRequestNoteMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new CustRequestNoteFound(foundCustRequestNotes);
