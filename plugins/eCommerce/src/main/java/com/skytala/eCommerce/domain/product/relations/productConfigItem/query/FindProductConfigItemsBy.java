...
public class FindProductConfigItemsBy extends Query {


Map<String, String> filter;
public FindProductConfigItemsBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<ProductConfigItem> foundProductConfigItems = new ArrayList<ProductConfigItem>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("productConfigItemId")) { 
 GenericValue foundElement = delegator.findOne("ProductConfigItem", false, filter);
...
throw new RecordNotFoundException(ProductConfigItem.class); 
 ...
 buf = delegator.findAll("ProductConfigItem", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundProductConfigItems.add(ProductConfigItemMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new ProductConfigItemFound(foundProductConfigItems);
