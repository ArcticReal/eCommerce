...
public class FindAllRespondingPartys extends Query {

@Override
public Event execute() {

Delegator delegator = DelegatorFactory.getDelegator("default");
List<RespondingParty> returnVal = new ArrayList<RespondingParty>();
try{
List<GenericValue> results = delegator.findAll("RespondingParty", false);
for (int i = 0; i < results.size(); i++) {
returnVal.add(RespondingPartyMapper.map(results.get(i)));
}
} catch (GenericEntityException e) {
 System.err.println(e.getMessage()); 
}
Event resultingEvent = new RespondingPartyFound(returnVal);
