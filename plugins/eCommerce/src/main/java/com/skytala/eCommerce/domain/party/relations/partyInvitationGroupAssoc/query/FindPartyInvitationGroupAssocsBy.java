...
public class FindPartyInvitationGroupAssocsBy extends Query {


Map<String, String> filter;
public FindPartyInvitationGroupAssocsBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<PartyInvitationGroupAssoc> foundPartyInvitationGroupAssocs = new ArrayList<PartyInvitationGroupAssoc>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("partyInvitationGroupAssocId")) { 
 GenericValue foundElement = delegator.findOne("PartyInvitationGroupAssoc", false, filter);
...
throw new RecordNotFoundException(PartyInvitationGroupAssoc.class); 
 ...
 buf = delegator.findAll("PartyInvitationGroupAssoc", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundPartyInvitationGroupAssocs.add(PartyInvitationGroupAssocMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new PartyInvitationGroupAssocFound(foundPartyInvitationGroupAssocs);
