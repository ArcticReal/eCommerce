package com.skytala.eCommerce.domain.accounting.relations.payment.query;
...
public class FindPaymentsBy extends Query {


Map<String, String> filter;
public FindPaymentsBy(Map<String, String> filter) {
this.filter = filter;
}
...
if(filter.size()==1&&filter.containsKey("paymentId")) { 
 GenericValue foundElement = delegator.findOne("Payment", false, filter);
...
throw new RecordNotFoundException(Payment.class); 
 ...
 buf = delegator.findAll("Payment", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundPayments.add(PaymentMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new PaymentFound(foundPayments);
