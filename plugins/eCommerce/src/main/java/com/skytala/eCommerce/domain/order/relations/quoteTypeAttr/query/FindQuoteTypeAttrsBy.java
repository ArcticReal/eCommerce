...
public class FindQuoteTypeAttrsBy extends Query {


Map<String, String> filter;
public FindQuoteTypeAttrsBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<QuoteTypeAttr> foundQuoteTypeAttrs = new ArrayList<QuoteTypeAttr>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("quoteTypeAttrId")) { 
 GenericValue foundElement = delegator.findOne("QuoteTypeAttr", false, filter);
...
throw new RecordNotFoundException(QuoteTypeAttr.class); 
 ...
 buf = delegator.findAll("QuoteTypeAttr", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundQuoteTypeAttrs.add(QuoteTypeAttrMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new QuoteTypeAttrFound(foundQuoteTypeAttrs);
