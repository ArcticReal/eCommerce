...
public class FindContentPurposeOperationsBy extends Query {


Map<String, String> filter;
public FindContentPurposeOperationsBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<ContentPurposeOperation> foundContentPurposeOperations = new ArrayList<ContentPurposeOperation>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("contentPurposeOperationId")) { 
 GenericValue foundElement = delegator.findOne("ContentPurposeOperation", false, filter);
...
throw new RecordNotFoundException(ContentPurposeOperation.class); 
 ...
 buf = delegator.findAll("ContentPurposeOperation", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundContentPurposeOperations.add(ContentPurposeOperationMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new ContentPurposeOperationFound(foundContentPurposeOperations);
