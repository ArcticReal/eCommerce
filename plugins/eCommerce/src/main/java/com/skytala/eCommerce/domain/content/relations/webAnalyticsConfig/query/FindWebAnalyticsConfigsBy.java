...
public class FindWebAnalyticsConfigsBy extends Query {


Map<String, String> filter;
public FindWebAnalyticsConfigsBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<WebAnalyticsConfig> foundWebAnalyticsConfigs = new ArrayList<WebAnalyticsConfig>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("webAnalyticsConfigId")) { 
 GenericValue foundElement = delegator.findOne("WebAnalyticsConfig", false, filter);
...
throw new RecordNotFoundException(WebAnalyticsConfig.class); 
 ...
 buf = delegator.findAll("WebAnalyticsConfig", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundWebAnalyticsConfigs.add(WebAnalyticsConfigMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new WebAnalyticsConfigFound(foundWebAnalyticsConfigs);
