...
public class FindValueLinkKeysBy extends Query {


Map<String, String> filter;
public FindValueLinkKeysBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<ValueLinkKey> foundValueLinkKeys = new ArrayList<ValueLinkKey>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("valueLinkKeyId")) { 
 GenericValue foundElement = delegator.findOne("ValueLinkKey", false, filter);
...
throw new RecordNotFoundException(ValueLinkKey.class); 
 ...
 buf = delegator.findAll("ValueLinkKey", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundValueLinkKeys.add(ValueLinkKeyMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new ValueLinkKeyFound(foundValueLinkKeys);
