...
public class FindWorkEffortFixedAssetStdsBy extends Query {


Map<String, String> filter;
public FindWorkEffortFixedAssetStdsBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<WorkEffortFixedAssetStd> foundWorkEffortFixedAssetStds = new ArrayList<WorkEffortFixedAssetStd>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("workEffortFixedAssetStdId")) { 
 GenericValue foundElement = delegator.findOne("WorkEffortFixedAssetStd", false, filter);
...
throw new RecordNotFoundException(WorkEffortFixedAssetStd.class); 
 ...
 buf = delegator.findAll("WorkEffortFixedAssetStd", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundWorkEffortFixedAssetStds.add(WorkEffortFixedAssetStdMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new WorkEffortFixedAssetStdFound(foundWorkEffortFixedAssetStds);
