...
public class FindSubscriptionTypeAttrsBy extends Query {


Map<String, String> filter;
public FindSubscriptionTypeAttrsBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<SubscriptionTypeAttr> foundSubscriptionTypeAttrs = new ArrayList<SubscriptionTypeAttr>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("subscriptionTypeAttrId")) { 
 GenericValue foundElement = delegator.findOne("SubscriptionTypeAttr", false, filter);
...
throw new RecordNotFoundException(SubscriptionTypeAttr.class); 
 ...
 buf = delegator.findAll("SubscriptionTypeAttr", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundSubscriptionTypeAttrs.add(SubscriptionTypeAttrMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new SubscriptionTypeAttrFound(foundSubscriptionTypeAttrs);
