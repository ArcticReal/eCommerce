...
public class ProductViewServiceController {
	...
	@PostMapping("/getProductCategoryMembers")
	@PreAuthorize(PERMIT_ALL)
	public ResponseEntity<List<ProductCategoryMember>> getProductCategoryMembers(HttpSession session,
															@RequestParam(value = "categoryId") String categoryId) {

		ProductCategory productCategory = new ProductCategory(categoryId, getFindProductCategorysQuery(categoryId));
		List<ProductCategoryMember> products = null;

		try {
			products = productCategory.getProductCategoryMembers(session);
		} catch (ServiceAuthException e) {
			return unauthorized();
		} catch (ServiceValidationException e) {
			return serverError();
