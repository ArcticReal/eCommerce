...
public class FindPaymentGatewayEwaysBy extends Query {


Map<String, String> filter;
public FindPaymentGatewayEwaysBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<PaymentGatewayEway> foundPaymentGatewayEways = new ArrayList<PaymentGatewayEway>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("paymentGatewayEwayId")) { 
 GenericValue foundElement = delegator.findOne("PaymentGatewayEway", false, filter);
...
throw new RecordNotFoundException(PaymentGatewayEway.class); 
 ...
 buf = delegator.findAll("PaymentGatewayEway", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundPaymentGatewayEways.add(PaymentGatewayEwayMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new PaymentGatewayEwayFound(foundPaymentGatewayEways);
