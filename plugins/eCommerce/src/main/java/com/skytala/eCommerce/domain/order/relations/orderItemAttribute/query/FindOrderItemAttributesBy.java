...
public class FindOrderItemAttributesBy extends Query {


Map<String, String> filter;
public FindOrderItemAttributesBy(Map<String, String> filter) {
this.filter = filter;
}
...
List<OrderItemAttribute> foundOrderItemAttributes = new ArrayList<OrderItemAttribute>();

try{
List<GenericValue> buf = new LinkedList<>();
if(filter.size()==1&&filter.containsKey("orderItemAttributeId")) { 
 GenericValue foundElement = delegator.findOne("OrderItemAttribute", false, filter);
...
throw new RecordNotFoundException(OrderItemAttribute.class); 
 ...
 buf = delegator.findAll("OrderItemAttribute", false); 
 }

for (int i = 0; i < buf.size(); i++) {
if(applysToFilter(buf.get(i))) {
foundOrderItemAttributes.add(OrderItemAttributeMapper.map(buf.get(i)));
}
}
...
Event resultingEvent = new OrderItemAttributeFound(foundOrderItemAttributes);
